Prompt Package Body MSAF_VETORIT_RELCFOMOD2_CPROC;
--
-- MSAF_VETORIT_RELCFOMOD2_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY msaf_vetorit_relcfomod2_cproc
IS
    ------------------------------------------------------------------------------
    -- Modifications:
    --  Who            When         What
    ...
    BEGIN
        ...
        lib_proc.add_param ( pstr
                           ,    LPAD ( ' '
                                     , 54
                                     , ' ' )
                             || '** Notas Fiscais por CFOP - Modelo 2 ** '
                           , 'varchar2'
                           , 'text'
                           , 'N'
                           , NULL
    ...
    IS
    BEGIN
        RETURN 'Relatório de Notas Fiscais por CFOP(Modelo 2)';
    END;
    ...
    BEGIN
        ...
        mproc_id :=
            lib_proc.new ( 'MSAF_VETORIT_RELCFOMOD2_CPROC'
                         , 48
                         , 150 );
        ...
        lib_proc.add_tipo ( mproc_id
                          , 2
                          , 'RELATORIO_DE_NOTAS_POR_CFO_MOD2.xls'
                          , 2 ); --2 arquivo
        ...
        linha_w := linha_w || 'Sub Serie NF' || vtab;
        linha_w := linha_w || 'NF AR' || vtab;
        linha_w := linha_w || 'Data Fiscal' || vtab;
        linha_w := linha_w || 'Produto' || vtab;
        linha_w := linha_w || 'Descricao' || vtab;
        linha_w := linha_w || 'Num Item NF' || vtab;
        linha_w := linha_w || 'Cod. Sit. Tributaria A-B' || vtab;
        linha_w := linha_w || 'Descricao Complementar' || vtab;
        linha_w := linha_w || 'CFOP' || vtab;
        linha_w := linha_w || 'Extensao CFOP' || vtab;
        linha_w := linha_w || 'NBM' || vtab;
        linha_w := linha_w || 'Pessoa Fisica Juridica' || vtab;
        linha_w := linha_w || 'Razao Social' || vtab;
        linha_w := linha_w || 'CNPJ CPF' || vtab;
        linha_w := linha_w || 'Valor Liquido' || vtab;
        linha_w := linha_w || 'Vlr Unit' || vtab;
        linha_w := linha_w || 'Quantidade' || vtab;
        linha_w := linha_w || 'Unidade Medida' || vtab;
        linha_w := linha_w || 'Vlr. ICMS' || vtab;
        linha_w := linha_w || 'Vlr. Base ICMS' || vtab;
        linha_w := linha_w || 'Aliq. ICMS' || vtab;
        linha_w := linha_w || 'ICMS ST' || vtab;
        linha_w := linha_w || 'Vlr. ICMS N Escrit.' || vtab;
        linha_w := linha_w || 'Vlr. IPI N Escrit.' || vtab;
        linha_w := linha_w || 'Vlr. IPI' || vtab;
        linha_w := linha_w || 'Vlr. Base IPI' || vtab;
        linha_w := linha_w || 'Aliq. IPI' || vtab;
        linha_w := linha_w || 'Mod. NF' || vtab;
        linha_w := linha_w || 'Chave de Acesso';
                ...
                FOR cur IN ( WITH dwt_w
                            ...
                            SELECT dwt_doc.cod_empresa
                                 ...
                                 , ( SELECT cpf_cgc
                                      ...
                                      WHERE ident_fis_jur = dwt_doc.ident_fis_jur )
                                       cpf_cgc
                                 , dwt_its.vlr_tot AS vlr_contab_item
                                 , dwt_doc.ind_situacao_esp
                                 , ( SELECT compl_cfop
                                       FROM x2081_extensao_cfo
                    ...
                    linha_w :=
                        ...
                        || lib_format.r2 ( COALESCE ( cur.vlr_contab_item
                                         ...
                                         , 18 )
                        || CHR ( 9 )
                        || lib_format.r2 ( COALESCE ( cur.vlr_unit
                                                    , 0 )
                                         , 18 )
                        || CHR ( 9 )
                        || cur.quantidade
                        ...
                        || lib_format.r2 ( COALESCE ( cur.vlr_base_icms_1
                                         ...
                                         , 18 )
                        || CHR ( 9 )
                        || lib_format.r2 ( COALESCE ( cur.aliq_tributo_icms
                                                    , 0 )
                                         , 18 )
                        || CHR ( 9 )
                        || lib_format.r2 ( COALESCE ( cur.vlr_tributo_icmss
                                                    , 0 )
                                         , 18 )
                        || CHR ( 9 )
                        || lib_format.r2 ( COALESCE ( cur.vlr_icms_ndestac
                                                    , 0 )
                                         , 18 )
                        || CHR ( 9 )
                        || lib_format.r2 ( COALESCE ( cur.vlr_ipi_ndestac
                                                    , 0 )
                                         , 18 )
                        || CHR ( 9 )
                        ...
                        || lib_format.r2 ( COALESCE ( cur.vlr_base_ipi_1
                                         ...
                                         , 18 )
                        || CHR ( 9 )
                        || lib_format.r2 ( COALESCE ( cur.aliq_tributo_ipi
                                                    , 0 )
                                         , 18 )
                        || CHR ( 9 )
                        || cur.cod_modelo
                        || CHR ( 9 )
                        || cur.num_autentic_nfe;
                    cont_w := cont_w + 1;
                    lib_proc.add ( plinha => linha_w
                                 , ppag => NULL
                                 , plin => NULL
        ...
        lib_proc.add_log ( LPAD ( '-'
                                ...
                                , '-' )
                         , 0 );
        lib_proc.add_log ( 'Log de Processo - MSAF_VETORIT_RELCFOMOD2_CPROC'
                         , 0 );
    ...
    EXCEPTION
        WHEN OTHERS THEN
            lib_proc.add_log ( 'Erro não previsto pelo sistema (MSAF_VETORIT_RELCFOMOD2_CPROC). ' || SQLERRM
                             , 0 );
