Prompt Package DPSP_FIN274REPMVA_CPROC;
--
-- DPSP_FIN274REPMVA_CPROC  (Package) 
--
CREATE OR REPLACE PACKAGE dpsp_fin274repmva_cproc
IS
    -- AUTOR    : DSP - GUILHERME
    -- DATA     : V7 CRIADA EM 17/OUT/2018
        ...
        SELECT   b.cod_empresa
            ...
            FROM ( SELECT   cod_empresa
                          ...
                          , TRUNC ( data_fiscal
                                  , 'MM' )
                                periodo
                          , SUM ( v_credito_substituido ) vlr_apurado
                       FROM msafi.dpsp_msaf_pmc_mva_sub
                      WHERE cod_empresa = mcod_empresa
                        AND cod_estab = p_i_cod_estab
                        AND data_fiscal BETWEEN p_i_data_ini AND p_i_data_fim
                        AND deb_cred_substituido =
                                DECODE ( p_i_filtro,  '2', 'CRÉDITO',  '3', 'DÉBITO',  deb_cred_substituido )
    ...
    IS
        SELECT   cod_empresa
               ...
               , DECODE ( lista_xml,  'P', 'POSITIVA',  'N', 'NEGATIVA',  'O', 'NEUTRA',  ' ' ) AS lista_xml
               , nf_brl_line_num_xml
               , inv_item_id_xml
               ...
               , v_vlr_base_icmsst_ret_xml
               , v_vlr_icmsst_ret_xml
               , v_credito_substituido
