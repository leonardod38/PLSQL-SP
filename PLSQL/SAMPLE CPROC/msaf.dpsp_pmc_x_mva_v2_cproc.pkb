Prompt Package Body DPSP_PMC_X_MVA_V2_CPROC;
--
-- DPSP_PMC_X_MVA_V2_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY dpsp_pmc_x_mva_v2_cproc
IS
    mproc_id INTEGER;
    ...
    IS
    BEGIN
        RETURN 'Processar Dados PMC x MVA (V2)';
    END;
    ...
    IS
    BEGIN
        RETURN 'Processar Carga de Dados para Ressarcimento PMC x MVA - NOVA VERSÃO';
    END;
            ...
            IF ( v_qtde > 0 ) THEN --**
                ...
                LOOP
                    ...
                    FOR c_part IN c_partition ( v_dt_part_fim ) LOOP
                        ...
                        v_sql := v_sql || '      AND NFE.NUM_CONTROLE_DOCTO  NOT LIKE ''C%''  ';
                        v_sql := v_sql || '      AND NFE.VLR_UNIT        > 0 ';
                        v_sql := v_sql || '      AND NFE.COD_NATUREZA_OP <> ''ISE'' ';
                        ---
                        v_sql := v_sql || '      AND NFE.IDENT_FIS_JUR = G.IDENT_FIS_JUR ';
    ...
    IS
    BEGIN
        DELETE msafi.dpsp_msaf_pmc_mva_v2
    ...
    BEGIN
        ---INSERIR RESULTADO
        v_sql := 'INSERT /*+APPEND*/ INTO MSAFI.DPSP_MSAF_PMC_MVA_V2 ';
        --
        ...
        v_sql := v_sql || ',DECODE(A.VLR_ICMS_ST_UNIT_E, 0, A.VLR_ICMS_ST_UNIT_AUX, A.VLR_ICMS_ST_UNIT_E) '; --VLR_ICMS_ST_UNIT_E
        v_sql := v_sql || ',A.STAT_LIBER_CNTR ';
        v_sql := v_sql || ',C.ALIQ_ST ';
        ...
        v_sql := v_sql || ',TRUNC(A.BASE_UNIT_S_VENDA*(TO_NUMBER(REPLACE(C.ALIQ_ST,''%'',''''))/100), 2) '; --VLR_ICMS_BRUTO
        v_sql :=
               v_sql
    ...
    BEGIN
        ...
        mproc_id :=
            lib_proc.new ( 'DPSP_PMC_X_MVA_V2_CPROC'
                         , 48
                         , 150 );
