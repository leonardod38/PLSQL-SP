Prompt Package Body DPSP_PERDAS_CD_CPROC;
--
-- DPSP_PERDAS_CD_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY dpsp_perdas_cd_cproc
IS
    ...
    IS
    BEGIN
        RETURN 'Processar Dados de PERDAS CDs';
    END;
    ...
    IS
    BEGIN
        RETURN 'Processar Carga de Dados para Relatorio de Perdas CDs';
    END;
    ...
    IS
        ...
        IS
            SELECT   loja
                   ...
                   , SUM (
                           ( CASE
                                WHEN adjust_type = 'I' THEN qty_base
                                WHEN adjust_type = 'D' THEN qty_base * -1
                                ELSE 0
                            END )
                     )
                         AS ajuste
                   , 0 AS saldo
                   , 0 AS custo
                FROM (SELECT /*+DRIVING_SITE(PS)*/
                            ps.business_unit AS loja
                           , ps.inv_item_id AS item
                           , ps.transaction_date AS data_inv
                           , ps.adjust_type
                           , ps.qty_base
                        FROM msafi.ps_transaction_inv ps
                       WHERE LTRIM ( REGEXP_REPLACE ( ps.business_unit
                                                    , 'V|D|L'
                                                    , '' )
                                   , '0' ) = LTRIM ( REGEXP_REPLACE ( p_i_cod_estab
                                                                    ...
                                                                    , '' )
                                                   , '0' )
                         AND ps.transaction_date BETWEEN p_i_data_ini AND p_i_data_fim
                         AND ps.transaction_group IN ( SELECT c.conteudo
                                                         FROM fpar_parametros b
                                                            , fpar_param_det c
                                                        WHERE b.nome_framework = 'DPSP_TRANSACAO_PARAM_CPAR'
                                                          AND b.id_parametros = c.id_parametro ))
            GROUP BY loja
                   , item
                   , data_inv;
    BEGIN
        ...
        IF ( v_qtde_inv > 0 ) THEN
            IF ( vp_inventario = 'S' ) THEN
                ...
                loga ( '[SOBREPOR ' || vp_cod_estab || ']'
                     , FALSE );
            ELSE ---JA EXISTE
                loga ( '[EXISTE TRANSACAO ' || vp_cod_estab || ']:' || v_qtde_inv
                     , FALSE );
            END IF;
        ELSE
    ...
    PROCEDURE envia_email ( mcod_empresa IN VARCHAR2
                          , vp_data_ini IN DATE
                          , vp_data_fim IN DATE
    ...
    BEGIN
        ...
        SELECT TRUNC (   (   (   86400
                           ...
                           / 60 )
                       / 60 )
                   24                * ( TRUNC ( ( ( ( 86400 * ( SYSDATE - TO_DATE ( vp_data_hora_ini , 'DD/MM/YYYY HH24:MI.SS' ) ) ) / 60 ) / 60 ) / 24 ) ) , TRUNC ( ( 86400 * ( SYSDATE - TO_DATE ( vp_data_hora_ini , 'DD/MM/YYYY HH24:MI.SS' ) ) ) / 60 ) - 60 * ( TRUNC ( ( ( 86400 * ( SYSDATE - TO_DATE ( vp_data_hora_ini , 'DD/MM/YYYY HH24:MI.SS' ) ) ) / 60 ) / 60 ) ) , TRUNC ( 86400 * ( SYSDATE - TO_DATE ( vp_data_hora_ini , 'DD/MM/YYYY HH24:MI.SS' ) ) ) - 60 * ( TRUNC ( ( 86400 * ( SYSDATE - TO_DATE ( vp_data_hora_ini , 'DD/MM/YYYY HH24:MI.SS' ) ) ) / 60 ) ) INTO v_horas , v_minutos , v_segundos FROM DUAL ;
...
v_tempo_exec := v_horas || ':' || v_minutos || '.' || v_segundos;
        ...
        IF ( vp_tipo = 'E' ) THEN
            ...
            v_txt_email := 'ERRO no Perdas CD !';
            v_txt_email := v_txt_email || CHR ( 13 ) || '>> Parâmetros: ';
            v_txt_email := v_txt_email || CHR ( 13 ) || ' - Empresa : ' || mcod_empresa;
...
v_txt_email :=
            ...
            v_assunto := 'Mastersaf -  Perdas CD apresentou ERRO';
            notifica ( ''
                     , 'S'
                     , v_assunto
                     , v_txt_email
        ...
        ELSE
            ...
            v_assunto := 'Mastersaf - Perdas CD Concluído';
            notifica ( 'S'
                     , ''
                     , v_assunto
                     , v_txt_email
    ...
    IS
        v_sql VARCHAR2 ( 1000 );
    BEGIN
        v_sql := 'DELETE MSAFI.DPSP_MSAF_PERDAS_CD ';
    ...
    PROCEDURE drop_old_tmp ( vp_proc_id IN NUMBER )
    IS
        CURSOR c_old_tmp
        IS
            SELECT table_name
            ...
            SELECT   x07.partition_name AS x07_partition_name
                ...
                FROM (SELECT b.owner
                           ...
                           , NVL (   LEAD ( b.partition_fim
                                 ...
                                 AS partition_ini
                           , b.partition_fim
                        FROM (/*SELECT A.OWNER, A.TABLE_NAME, A.PARTITION_NAME, TO_DATE(A.PARTITION_FIM,'DD/MM/YYYY') AS PARTITION_FIM*/
                              SELECT   a.owner
                                     , a.table_name
                                     , a.partition_name
                                  ...
                                  FROM TABLE ( msafi.dpsp_recupera_particao ( 'MSAF'
                                                                            ...
                                                                            , TO_DATE ( '20130101'
                                                                                      , 'YYYYMMDD' )
                                                                            , p_i_data_final ) ) a
                              /*ORDER BY TO_DATE(A.PARTITION_FIM,'DD/MM/YYYY')*/
    ...
    IS
        ...
        p_compra_direta VARCHAR ( 1 );
        p_cd1 VARCHAR2 ( 6 );
        p_cd2 VARCHAR2 ( 2 );
        p_cd3 VARCHAR2 ( 2 );
        p_cd4 VARCHAR2 ( 2 );
        p_filiais VARCHAR2 ( 2 );
        p_uf VARCHAR2 ( 2 );
        p_inventario VARCHAR2 ( 2 );
    BEGIN
        ...
        mproc_id :=
            lib_proc.new ( 'DPSP_PERDAS_CD_CPROC'
                         , 48
                         , 150 );
        ...
        loga ( '[>>INICIO DO PROCESSAMENTO...:' || p_proc_instance || '<<]'
             , FALSE );
        loga ( '[DT INICIAL][' || v_data_inicial || '][DT FINAL][' || v_data_final || ']'
             , FALSE );
        ...
        --PREPARAR LOJAS
        IF ( p_lojas.COUNT > 1 ) THEN
            --  RAISE_APPLICATION_ERROR(-20001,'SELECIONE UM CD POR VEZ!');
            lib_proc.add (
                           '>>>>>>>>>>>[NÃO É PERMITIDO EXECUTAR MAIS DE UM CD]- SELECIONE UM CD POR VEZ! <<<<<<<<<<<<<<<'
            );
            lib_proc.close;
            COMMIT;
            RETURN mproc_id;
        ...
        p_cd1 := a_estabs_full ( 1 );
        p_origem1 := '2';

        --CHECAR SE CDs SAO VALIDOS-INI-------------------------------------------
        SELECT cgc
          INTO a_cod_empresa ( p_cd1 ).cnpj
          FROM msaf.estabelecimento
         WHERE cod_empresa = msafi.dpsp.empresa
           AND cod_estab = p_cd1;

        a_cod_empresa ( p_cd1 ).empresa := msafi.dpsp.empresa;
        a_cod_empresa ( p_cd1 ).dblink := 'LOCAL';

        SELECT bu_po1
          INTO a_cod_empresa ( p_cd1 ).business_unit
          FROM msafi.dsp_interface_setup
         WHERE cod_empresa = msafi.dpsp.empresa;
        ...
        loga ( '[VERIFICACAO DE TRANSACAO][END]'
             , FALSE );
        ...
        FOR est IN a_estabs_full.FIRST .. a_estabs_full.COUNT --(99)
            ...
            ELSIF ( p_carga <> 'S' ) THEN
                ...
                INSERT INTO msafi.dpsp_msaf_perdas_cd ( cod_empresa
                                                      , cod_estab
                                                      , data_inv
                                                      , usuario
                                                      , dat_operacao )
            ...
            IF p_carga <> 'S' --(88)
                ...
                IF ( v_qtde_check > 0 ) THEN --(999)
                    ...
                    dbms_application_info.set_module ( v_module
                                                     , '17 INSERT FINAL' );
                    v_sql_resultado := 'INSERT /*+APPEND*/ INTO MSAFI.DPSP_MSAF_PERDAS_CD ( ';
