Prompt Package Body DPSP_REL_CARTOES_CPROC;
--
-- DPSP_REL_CARTOES_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY dpsp_rel_cartoes_cproc
IS
    FUNCTION parametros
        RETURN VARCHAR2
    IS
    ...
    IS
    BEGIN
        RETURN 'Relatório de Crédito dos Cartões';
    END;
    ...
    IS
    BEGIN
        RETURN 'Emitir relatório de crédito para vendas com cartão de crédito / débito';
    END;
    ...
    BEGIN
        ...
        mcod_empresa := lib_parametros.recuperar ( 'EMPRESA' );
        mproc_id :=
            lib_proc.new ( 'DPSP_REL_CARTOES_CPROC'
                         , 48
                         , 150 );
        ...
        IF ( p_conf = 'S' ) THEN --(1)
            ...
            FOR i IN 1 .. a_estabs.COUNT --(5)
                ...
                v_text01 := v_text01 || '  FROM MSAFI.DPSP_MSAF_CARTOES A, MSAFI.DSP_ESTABELECIMENTO B ';
                v_text01 :=
                       v_text01
