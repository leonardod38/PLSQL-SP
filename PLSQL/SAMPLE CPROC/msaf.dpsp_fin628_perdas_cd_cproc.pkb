Prompt Package Body DPSP_FIN628_PERDAS_CD_CPROC;
--
-- DPSP_FIN628_PERDAS_CD_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY dpsp_fin628_perdas_cd_cproc
IS
    mproc_id INTEGER;
    ...
    IS
    BEGIN
        RETURN 'Processar Dados Perdas - CDs';
    END;
    ...
    IS
    BEGIN
        RETURN 'Processar Carga de Dados para Dados Perdas - CDs';
    END;
    ...
    BEGIN
        ...
        v_sql := v_sql || 'PCTFREE 10 TABLESPACE MSAF_WORK_TABLES ';
    ...
    BEGIN
        ...
        v_sql := v_sql || '  ) ';
        v_sql := v_sql || '  PCTFREE 10 NOLOGGING TABLESPACE MSAF_WORK_INDEXES ';
    ...
    BEGIN
        ...
        v_sql :=
               v_sql
            || '                           ORDER BY X08.DATA_FISCAL DESC, X07.DATA_EMISSAO DESC, X08.NUM_DOCFIS DESC, X08.DISCRI_ITEM DESC, X08.SERIE_DOCFIS DESC) RANK ';
    ...
    BEGIN
        ...
        v_sql := v_sql || ' ) TABLESPACE MSAF_WORK_TABLES ';
    ...
    BEGIN
        ...
        IF ( vp_cod_estab = 'DP906' ) THEN
            ...
            v_sql := v_sql || '   AND NOT EXISTS (SELECT ''Y'' FROM MSAFI.DPSP_MSAF_ANTECIPACAO ANT ';
            v_sql := v_sql || '                   WHERE ANT.COD_ESTAB = ''DP906'' ';
            v_sql := v_sql || '                     AND ANT.NUM_CONTROLE_DOCTO = E.NUM_CONTROLE_DOCTO ';
            v_sql := v_sql || '                     AND ANT.NUM_ITEM = E.NUM_ITEM ) ';
            ---
            ...
            v_sql := v_sql || '	  E.DISCRI_ITEM      ,';
            ---
            v_sql := v_sql || '	  A.COD_ESTAB AS BUSINESS_UNIT, ';
            v_sql := v_sql || '	  A.NUM_CONTROLE_DOCTO AS NF_BRL_ID, ';
            v_sql := v_sql || '	  A.NUM_ITEM AS NF_BRL_LINE_NUM, ';
            v_sql := v_sql || '	  A.VLR_ANTECIP_IST  , ';
            v_sql := v_sql || '	  A.VLR_ANTECIP_REV	   ';
            v_sql := v_sql || ' FROM MSAFI.DPSP_MSAF_ANTECIPACAO A, ';
            v_sql := v_sql || ' ' || vp_tabela_class || ' E ';
            v_sql := v_sql || ' WHERE E.CLASSIFICACAO   = 5 ';
            v_sql := v_sql || '   AND A.COD_ESTAB = ''DP906'' ';
            v_sql := v_sql || '   AND A.NUM_CONTROLE_DOCTO = E.NUM_CONTROLE_DOCTO ';
            v_sql := v_sql || '   AND A.NUM_ITEM = E.NUM_ITEM ';
        ELSE
            v_sql := ' SELECT /*+DRIVING_SITE(A)*/ ';
            v_sql := v_sql || '	  E.PROC_ID          ,';
            v_sql := v_sql || '   E.DATA_FISCAL_S    ,';
    ...
    BEGIN
        ...
        mproc_id :=
            lib_proc.new ( 'DPSP_FIN628_PERDAS_CD_CPROC'
                         , 48
                         , 150 );
