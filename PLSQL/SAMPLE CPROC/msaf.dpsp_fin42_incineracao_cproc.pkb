Prompt Package Body DPSP_FIN42_INCINERACAO_CPROC;
--
-- DPSP_FIN42_INCINERACAO_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY dpsp_fin42_incineracao_cproc
IS
    mproc_id INTEGER;
    ...
    mnm_cproc VARCHAR2 ( 100 ) := 'Processar Carga de Ressarcimento de ST para NF de Incineração';
    mds_cproc VARCHAR2 ( 100 ) := 'Processo Carga de Ressarcimento de ST para NF de Incineração';
    ...
    BEGIN
        ...
        IF ( vp_tipo = 'E' ) THEN --VP_TIPO = 'E' (ERRO) OU 'S' (SUCESSO)
            ...
            v_txt_email := v_txt_email || CHR ( 13 ) || ' - Data Início : ' || LAST_DAY ( vp_data_ini );
            ...
            v_assunto := 'Mastersaf - Ressarcimento de ST para NF de Incineração apresentou ERRO';
        --NOTIFICA('', 'S', V_ASSUNTO, V_TXT_EMAIL, 'DPSP_RES_INTER_CPROC');

        ELSE
            v_txt_email := 'Processo de Ressarcimento de ST para NF de Incineração finalizado com SUCESSO.';
            v_txt_email := v_txt_email || CHR ( 13 ) || '>> Parâmetros: ';
            v_txt_email := v_txt_email || CHR ( 13 ) || ' - Empresa : ' || vp_cod_empresa;
            v_txt_email := v_txt_email || CHR ( 13 ) || ' - Data Início : ' || LAST_DAY ( vp_data_ini );
            v_txt_email := v_txt_email || CHR ( 13 ) || ' - Data Fim : ' || vp_data_fim;
            ...
            v_txt_email :=
                ...
                || TO_CHAR ( SYSDATE
                           , 'DD/MM/YYYY HH24:MI.SS' );
            v_txt_email := v_txt_email || CHR ( 13 ) || ' - Tempo Execução : ' || v_tempo_exec;
            v_assunto := 'Mastersaf - Ressarcimento de ST para NF de Incineração Concluído';
        --NOTIFICA('S', '', V_ASSUNTO, V_TXT_EMAIL, 'DPSP_RES_INTER_CPROC');
    ...
    BEGIN
        ...
        v_sql := v_sql || 'LISTAPRODUTO         VARCHAR2(20), ';
        ...
        v_sql := v_sql || 'IDENT_NBM           NUMBER(5),   ';
        ...
        v_sql := v_sql || 'ESTORNO_PIS_S       NUMBER(17,2), ';
        v_sql := v_sql || 'ESTORNO_COFINS_S    NUMBER(17,2), ';
    ...
    BEGIN
        ...
        v_sql := v_sql || ' COD_SITUACAO_A       VARCHAR2(2), ';
    ...
    PROCEDURE create_tab_people ( vp_proc_instance IN NUMBER
                                , vp_tab_people   OUT VARCHAR2 )
    IS
        v_sql VARCHAR2 ( 2000 );
    BEGIN
        ---CRIAR TEMP DE ENTRADA EM CD
        vp_tab_people := 'DPSP_PEOPLE_' || vp_proc_instance;

        v_sql := 'CREATE TABLE ' || vp_tab_people || ' ( ';
        v_sql := v_sql || ' BUSINESS_UNIT       VARCHAR2(6), ';
        v_sql := v_sql || ' NF_BRL_ID           VARCHAR2(10), ';
        v_sql := v_sql || ' TXN_NAT_BBL         VARCHAR2(50), ';
        v_sql := v_sql || ' NFEE_KEY_BBL        VARCHAR2(44)) ';
        v_sql := v_sql || ' PCTFREE     10 ';
    ...
    PROCEDURE load_people ( vp_proc_instance IN VARCHAR2
                          , vp_cod_estab IN VARCHAR2
                          , vp_data_ini IN DATE
                          , vp_data_fim IN DATE
                          , vp_tab_people IN VARCHAR2
                          , vp_data_hora_ini IN VARCHAR2 )
    IS
        --RANGE DE DATAS PARA BUSCAR VENDAS
        v_data_inicial DATE
            :=   TRUNC ( vp_data_ini )
               - (   TO_NUMBER ( TO_CHAR ( vp_data_ini
                                         , 'DD' ) )
                   - 1 ); -- DATA INICIAL
        v_data_final DATE := LAST_DAY ( vp_data_fim ); -- DATA FINAL
    ...
    BEGIN
        FOR cd IN c_data_saida ( v_data_inicial
                               , v_data_final ) LOOP
            v_sql := 'INSERT INTO ' || vp_tab_people || ' ( ';
            v_sql :=
                v_sql || 'select BUSINESS_UNIT, NF_BRL_ID, TXN_NAT_BBL, NFEE_KEY_BBL from msafi.ps_nf_hdr_bbl_fs nfh ';
            v_sql := v_sql || 'where nfh.txn_nat_bbl  = ''REMESSA PARA INCINERACAO'' ';
            v_sql :=
                v_sql || 'and CASE WHEN NFH.LT_GRP_ID_BBL IN (''TRO_LIB_23'', ''TRO_DIA_23'') THEN NFH.ENTERED_DT ';
            v_sql :=
                   v_sql
            ...
            v_sql := v_sql || '             END =   TO_DATE(''' || cd.data_normal || ''',''DD/MM/YYYY'') ';

            IF mcod_empresa = 'DSP' THEN
                v_sql :=
                       v_sql
                    || '             and nfh.business_unit  = REPLACE('''
                    || vp_cod_estab
                    || ''',''DSP'',''VD''))';
            ELSE
                v_sql :=
                    v_sql || '             and nfh.business_unit  = REPLACE(''' || vp_cod_estab || ''',''DP'',''VD''))';
            END IF;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    raise_application_error ( -20022
                                            , '!ERRO INSERT LOAD_PEOPLE!' );
            END;
        END LOOP;

        loga ( 'LOAD_PEOPLE-FIM-' || vp_cod_estab
             , FALSE );
    END;
    ...
    BEGIN
        FOR cd IN c_data_saida ( v_data_inicial
            ...
            v_sql := v_sql || ' SELECT /*+driving_site(ps)*/ ' || vp_proc_instance || ', ';
            ...
            v_sql :=
                   v_sql
                || ' CASE WHEN S.LISTA = ''N'' THEN ''Negativo''  WHEN S.LISTA = ''P'' THEN ''Positivo'' WHEN S.LISTA = ''O'' THEN ''Neutro'' ELSE '''' END AS LISTA_PRODUTO , ';
            ...
            v_sql := v_sql || ' K.IDENT_NBM AS NCM, ';
            v_sql := v_sql || ' B.QUANTIDADE AS QTDE_SAIDA, ';
            v_sql := v_sql || ' round((nvl(B.VLR_ITEM,0) - nvl(B.VLR_DESCONTO,0))/b.QUANTIDADE,4) AS VLR_UNIT, ';
            ...
            v_sql := v_sql || '        (B.VLR_PIS*B.QUANTIDADE) AS ESTORNO_PIS_S, ';
            v_sql := v_sql || '        (B.VLR_COFINS*B.QUANTIDADE) AS ESTORNO_COFINS_S, ';
            v_sql := v_sql || '        round((nvl(B.VLR_ITEM,0) - nvl(B.VLR_DESCONTO,0))/b.QUANTIDADE,4) AS VLR_UNIT, ';
            ...
            v_sql := v_sql || '      MSAF.dpsp_ps_lista S, ';
            v_sql := v_sql || '      ' || vp_tab_people || ' ps ';
            ...
            v_sql := v_sql || '   AND A.DATA_FISCAL = TO_DATE(''' || cd.data_normal || ''',''DD/MM/YYYY'') ';
            ...
            v_sql := v_sql || '   AND PS.txn_nat_bbl      = ''REMESSA PARA INCINERACAO'' ';
            v_sql := v_sql || '   AND a.num_controle_docto  = ps.nf_brl_id ';

            IF mcod_empresa = 'DSP' THEN
                v_sql := v_sql || '   and PS.business_unit = REPLACE(''' || vp_cod_estab || ''',''DSP'',''VD'') ';
            ELSE
                v_sql := v_sql || '   and PS.business_unit = REPLACE(''' || vp_cod_estab || ''',''DP'',''VD'') ';
            END IF;
            ...
            v_sql := v_sql || '   AND S.COD_PRODUTO = H.COD_PRODUTO ';
            v_sql := v_sql || '   AND S.EFFDT = (SELECT MAX (SS.EFFDT) FROM MSAF.dpsp_ps_lista SS ';
            v_sql := v_sql || '   WHERE SS.COD_PRODUTO = S.COD_PRODUTO ';
            v_sql := v_sql || '   AND SS.LISTA = S.LISTA ';
            v_sql := v_sql || '   AND EFFDT <= SYSDATE)) ';
    ...
    BEGIN
        ...
        v_sql := v_sql || '              G.RAZAO_SOCIAL AS RAZAO_SOCIAL_E, ';
        v_sql := v_sql || '              A.COD_NBM, ';
        ...
        v_sql := v_sql || '              I.COD_SITUACAO_A AS COD_SITUACAO_A, ';
        ...
        v_sql :=
               v_sql
            || '          AND B.COD_CFO             IN (''1102'',''2102'',''1403'',''2403'',''1910'',''2910'',''1917'',''2917'') ';
        ...
        v_sql := v_sql || '          AND X08.IDENT_SITUACAO_B   = I.IDENT_SITUACAO_A ';
        ...
        LOOP
            ...
            FOR i IN 1 .. tab_e.COUNT LOOP
                v_insert :=
                    ...
                    || tab_e ( i ).cpf_cgc
                    || ''','''
                    || REPLACE ( tab_e ( i ).razao_social_e
                               , ''''
                               , ' ' )
                    || ''' ,'''
                    || tab_e ( i ).cod_nbm
    ...
    BEGIN
        ...
        IF ( vp_cod_estab = 'DP906' ) THEN
            ...
            v_sql := v_sql || ' WHERE A.DATA_FISCAL     = E.DATA_FISCAL ';
        ...
        ELSE
            ...
            v_sql := v_sql || '    WHERE  A.BUSINESS_UNIT   IN (P.BU_PO1, P.BU_PO2, P.BU_PO3, P.BU_PO4, P.BU_PO5) ';
            v_sql := v_sql || '   AND A.NF_BRL_ID       = E.NUM_CONTROLE_DOCTO ';
            v_sql := v_sql || '   AND A.NF_BRL_LINE_NUM = E.NUM_ITEM ';
    ...
    BEGIN
        ...
        IF ( vp_cod_estab <> 'DP906' ) THEN
            ...
            v_sql := v_sql || '   AND P.COD_EMPRESA     = MSAFI.DPSP.EMPRESA ';
            v_sql :=
                v_sql || ' AND NOT EXISTS (SELECT ''X'' FROM ' || vp_tabela_gare || ' K WHERE K.PROC_ID = PROC_ID ';
            v_sql := v_sql || '   AND K.DATA_FISCAL_S = DATA_FISCAL_S ';
            v_sql := v_sql || '   AND K.COD_EMPRESA = COD_EMPRESA ';
            v_sql := v_sql || '   AND K.COD_ESTAB = COD_ESTAB ';
            v_sql := v_sql || '   AND K.DATA_FISCAL = DATA_FISCAL ';
            v_sql := v_sql || '   and K.MOVTO_E_S =   MOVTO_E_S ';
            v_sql := v_sql || '   And K.NORM_DEV =  NORM_DEV ';
            v_sql := v_sql || '   and K.IDENT_DOCTO =  IDENT_DOCTO ';
            v_sql := v_sql || '   and K.IDENT_FIS_JUR = IDENT_FIS_JUR ';
            v_sql := v_sql || '   and K.NUM_DOCFIS =  NUM_DOCFIS ';
            v_sql := v_sql || '   and K.SERIE_DOCFIS =  SERIE_DOCFIS ';
            v_sql := v_sql || '   and K.SUB_SERIE_DOCFIS = SUB_SERIE_DOCFIS ';
            v_sql := v_sql || '   and K.DISCRI_ITEM = DISCRI_ITEM) ';
    ...
    BEGIN
        ...
        v_sql := v_sql || 'ESTORNO_PIS_E        NUMBER(17,4), ';
        v_sql := v_sql || 'ESTORNO_COFINS_E     NUMBER(17,4), ';
        ...
        v_sql := v_sql || ',S.LISTAPRODUTO       ';
        v_sql := v_sql || ',S.ESTORNO_PIS_S      ';
        v_sql := v_sql || ',S.ESTORNO_COFINS_S   ';
        v_sql := v_sql || ',S.VLR_PIS            ';
        v_sql := v_sql || ',S.VLR_COFINS         ';
        ---
        v_sql := v_sql || ',E.COD_ESTAB          ';
        v_sql := v_sql || ',E.DATA_FISCAL        ';
        v_sql := v_sql || ',E.MOVTO_E_S          ';
        ...
        v_sql := v_sql || ' ,E.VLR_UNIT*(1.65/100) AS ESTORNO_PIS_E	';
        v_sql := v_sql || ' ,E.VLR_UNIT*(7.6/100) AS ESTORNO_COFINS_E ';
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE
            'INSERT /*+APPEND*/ INTO MSAFI.DPSP_MASF_INCINERACAO (SELECT * FROM ' || vp_tabela_inter_tmp || ' )';
        ...
        EXECUTE IMMEDIATE
               'INSERT /*+APPEND*/ INTO MSAFI.DPSP_MASF_INCINERACAO ( '
            || 'SELECT COD_EMPRESA, COD_ESTAB, NUM_DOCFIS, NUM_CONTROLE_DOCTO, NUM_ITEM,COD_PRODUTO, DESCR_ITEM, '
            || '       DATA_FISCAL, UF_ORIGEM, UF_DESTINO, COD_FIS_JUR, CNPJ, RAZAO_SOCIAL, SERIE_DOCFIS, '
            || '       CFOP, FINALIDADE, NBM, NUM_AUTENTIC_NFE, QUANTIDADE, VLR_UNIT , VLR_ITEM, VLR_BASE_ICMS , VLR_ICMS , ALIQ_ICMS ,LISTAPRODUTO, ESTORNO_PIS_S, ESTORNO_COFINS_S, VLR_PIS, VLR_COFINS, '
            || '       '''', NULL, '''', '''', '''', '''', '''', '''', '''', '''', 0, '''', '''', '''','''', '''', '''', 0, 0, 0, '''', NULL, '''', '''', '
        ...
        loga ( 'INS2-FIM '
             , FALSE );
    ---DBMS_STATS.GATHER_TABLE_STATS('MSAFI', 'DPSP_MASF_INCINERACAO');
    ...
    IS
        ...
        vp_tab_people VARCHAR2 ( 30 );
        vp_cod_estab VARCHAR2 ( 30 );
        bloc VARCHAR2 ( 1 );
        ---
    ...
    BEGIN
        ...
        mproc_id :=
            lib_proc.new ( 'DPSP_FIN42_INCINERACAO_CPROC'
                         , 48
                         , 150 );
        ...
        lib_proc.add_header ( 'Executar processamento de Ressarcimento de ST para NF de Incineração'
                            , 1
                            , 1 );
        lib_proc.add ( ' ' );
        ...
        IF LAST_DAY ( v_data_inicial ) = LAST_DAY ( v_data_final ) THEN
            ...
            FOR est IN a_estabs.FIRST .. a_estabs.COUNT --(1)
                ...
                IF v_validar_status = 0 THEN
                    loga ( '>> INICIO CD: ' || a_estabs ( est ) || ' PROC INSERT ' || p_proc_instance
                         , FALSE );
                    ...
                    IF v_qtd = 0 THEN
                        ...
                        lib_proc.add ( 'CD ' || a_estabs ( est ) || ' sem dados na origem.' );

                        bloc := 0;

                        lib_proc.add ( ' ' );
                        loga ( '---CD ' || a_estabs ( est ) || ' - SEM DADOS DE ORIGEM---'
                             , FALSE );
                    ...
                    ELSE
                        ...
                        IF LAST_DAY ( v_data_inicial ) < LAST_DAY ( SYSDATE ) THEN
                            msaf.dpsp_suporte_cproc_process.inserir_status_rel ( mcod_empresa
                                                                               ...
                                                                               , musuario
                                                                               , v_data_hora_ini );
                            lib_proc.add ( 'CD ' || a_estabs ( est ) || ' - Período Encerrado' );
                        ...
                        ELSE
                            ...
                            lib_proc.add ( 'CD ' || a_estabs ( est ) || ' - PERIODO EM ABERTO,'
                                         , 1 );
                            ...
                            lib_proc.add ( ' '
                                         , 1 );
                            loga ( '---CD ' || a_estabs ( est ) || ' - PERIODO EM ABERTO---'
                                 , FALSE );
                        END IF;
                    END IF;
                --PERIODO JÁ ENCERRADO
                ELSE
                    lib_proc.add ( 'CD ' || a_estabs ( est ) || ' - Período já processado e encerrado' );
                ...
                IF bloc <> '1' THEN
                    ...
                    --CRIA TABELA TEMPORARIA COM DADOS DO PEOPLE
                    create_tab_people ( p_proc_instance
                                      , vp_tab_people );

                    --CARREGAR TABELA DO PEOPLE
                    load_people ( p_proc_instance
                                , a_estabs ( est )
                                , p_data_ini
                                , p_data_fim
                                , vp_tab_people
