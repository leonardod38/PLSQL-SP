Prompt Package DPSP_REL_PMC_X_MVA_CPROC;
--
-- DPSP_REL_PMC_X_MVA_CPROC  (Package) 
--
CREATE OR REPLACE PACKAGE dpsp_rel_pmc_x_mva_cproc
IS
    -- AUTOR    : REBELLO
    ...
    IS
        SELECT   b.cod_empresa
               ...
               , NVL ( a.vlr_apurado_xml, 0 ) vlr_apurado_xml
            FROM (SELECT   cod_empresa
                         , cod_estado
                         , cod_estab
                         , SUM ( NVL ( DECODE ( vlr_icms_st_unit_e, 0, 0, vlr_dif_qtde ), 0 ) ) vlr_apurado
                         , SUM ( NVL ( DECODE ( vlr_icms_st_unit_e, 0, 0, vlr_dif_qtde_xml ), 0 ) ) vlr_apurado_xml
    ...
    IS
        SELECT   b.cod_empresa
               ...
               , -- nvl(a.vlr_pmc, 0) vlr_pmc,
                 ...
                 SUM ( vlr_dif_qtde ) vlr_dif_qtde
               , SUM ( vlr_dif_qtde_xml ) vlr_dif_qtde_xml
            FROM (SELECT   cod_empresa
                         , cod_estado
                         , cod_estab
                         , TO_CHAR ( data_fiscal
                         ...
                         , --                SUM(CASE
                           ...
                           --                END) vlr_pmc,
                           --            SUM(CASE
                           --                  WHEN (vlr_pmc <= 0 OR vlr_pmc IS NULL OR vlr_pmc = '') THEN
                           --                  nvl(decode(vlr_icms_st_unit_e, 0, 0, vlr_dif_qtde), 0)
                           --                  ELSE
                           --                   0
                           --                END) vlr_mva,
                  ...
                  SELECT   cod_empresa
                         ...
                         , TO_CHAR ( data_fiscal
                               ...
                               periodo
                         , 0 vlr_dif_qtde
                         , NVL ( SUM ( vlr_dif_qtde_xml ), 0 ) vlr_dif_qtde_xml
                      FROM msafi.dpsp_msaf_pmc_mva
                     WHERE cod_empresa = mcod_empresa
                       AND cod_estab = p_i_cod_estab
                       AND data_fiscal BETWEEN p_i_data_ini AND p_i_data_fim
                       AND ( CASE
                                WHEN vlr_dif_qtde_xml < 0 THEN 'DÉBITO'
                                WHEN NVL ( vlr_dif_qtde_xml, 0 ) = 0 THEN '-'
                                WHEN vlr_dif_qtde_xml > 0 THEN 'CRÉDITO'
                            END ) = 'CRÉDITO'
                  GROUP BY cod_empresa
                         , cod_estado
                         , cod_estab
    ...
    IS
        SELECT   cod_empresa
               ...
               , NVL ( vlr_dif_qtde_xml, 0 ) vlr_dif_qtde_xml
               , ( CASE
                      WHEN vlr_dif_qtde_xml < 0 THEN 'DÉBITO'
                      WHEN NVL ( vlr_dif_qtde_xml, 0 ) = 0 THEN '-'
                      WHEN vlr_dif_qtde_xml > 0 THEN 'CRÉDITO'
