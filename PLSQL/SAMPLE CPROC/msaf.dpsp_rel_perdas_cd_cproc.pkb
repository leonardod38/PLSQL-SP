Prompt Package Body DPSP_REL_PERDAS_CD_CPROC;
--
-- DPSP_REL_PERDAS_CD_CPROC  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY dpsp_rel_perdas_cd_cproc
IS
    FUNCTION parametros
        RETURN VARCHAR2
    IS
    ...
    BEGIN
        ...
        lib_proc.add_param (
                           ...
                           || INITCAP(A.CIDADE) FROM ESTABELECIMENTO A, ESTADO B, MSAFI.DSP_ESTABELECIMENTO C WHERE A.COD_EMPRESA  = '''
                             || mcod_empresa
                             || ''' AND B.IDENT_ESTADO = A.IDENT_ESTADO AND A.COD_EMPRESA = C.COD_EMPRESA AND A.COD_ESTAB = C.COD_ESTAB AND C.TIPO = ''C'' AND B.COD_ESTADO LIKE :6 ORDER BY B.COD_ESTADO, A.COD_ESTAB'
        );
    ...
    IS
    BEGIN
        RETURN 'Relatório de PERDAS CDs';
    END;
    ...
    IS
    BEGIN
        RETURN 'Relatório de Ressarcimento sobre Perdas CDs (Sintético, Analítico e Mapa)';
    END;
    ...
    BEGIN
        ...
        mcod_empresa := lib_parametros.recuperar ( 'EMPRESA' );
        mproc_id :=
            lib_proc.new ( 'DPSP_REL_PERDAS_CD_CPROC'
                         , 48
                         , 150 );
        ...
        IF ( p_sintetico = 'S' ) THEN
            ...
            lib_proc.add ( dsp_planilha.linha (
                                                p_conteudo =>    dsp_planilha.campo ( 'CD' )
                                                              || --
                    ...
                    v_text01 :=
                        dsp_planilha.linha (
                                             p_conteudo =>    dsp_planilha.campo ( cr_s.cod_estado )
                                                           ...
                                                           || --
                                                             dsp_planilha.campo (
                                                                                  dsp_planilha.texto ( cr_s.periodo )
                                                              )
                                                           || --
