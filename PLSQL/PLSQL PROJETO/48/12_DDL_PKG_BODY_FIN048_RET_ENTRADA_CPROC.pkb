CREATE OR REPLACE PACKAGE BODY MSAF.DPSP_FIN048_RET_ENTRADA_CPROC IS
  mproc_id     NUMBER;
  vn_linha     NUMBER := 0;
  vn_pagina    NUMBER := 0;
  ...
  v_data_hora_ini VARCHAR2(20) := to_char(SYSDATE, 'DD/MM/YYYY HH24:MI.SS');
      ...
      FOR j IN x08_itens_merc_typ.FIRST .. x08_itens_merc_typ.LAST LOOP
        ...
        BEGIN
          ...
          BEGIN
            ...
            loga('Recupera itens_trib:' || x08t_type.cod_tributo || ';' ||
            ...
            EXCEPTION
              WHEN OTHERS THEN
                ...
                INSERT INTO msafi.x07_docto_fiscal_log VALUES log_x07_type;
  ...
  BEGIN
    ...
    lib_proc.add_param(pparam      => pstr, --p_radio
                       ptitulo     => 'Processo',
                       ptipo       => 'VARCHAR2',
  ...
  BEGIN
    ...
    IF last_day(pdt_ini) = last_day(pdt_fim)
    THEN
      --=================================================================================
      -- INICIO
      --=================================================================================
      ...
      FOR cd IN lista_cds LOOP
        ...
        loga($$PLSQL_UNIT, FALSE);
      
        IF p_radio = 1
        THEN
          --PROCESSAR
        
          v_validar_status := msaf.dpsp_suporte_cproc_process.validar_status_rel(mcod_empresa,
                                                                                 cd.cod_estab,
          ...
          --=================================================================================
          IF v_validar_status = 0
          THEN
            loga('>> INICIO CD: ' || cd.cod_estab || ' PROC INSERT ' ||
                 p_proc_instance,
                 FALSE);
          
            loga('PASSO 2  : ' || v_validar_status, FALSE);
            ---------------------
            -- LIMPEZA
            ---------------------
            ...
            loga('::LIMPEZA DOS REGISTROS ANTERIORES (MSAFI.DPSP_FIN048_RET_NF_ENT), CD: ' ||
            ...
            ELSE
              ...
              ---------------------
              IF last_day(pdt_ini) < last_day(SYSDATE)
              THEN
                ...
                v_retorno_status := msaf.dpsp_suporte_cproc_process.retornar_status_rel(mcod_empresa,
                                                                                        cd.cod_estab,
                                                                                        to_number(to_char(pdt_ini,
                                                                                                          'YYYYMM')),
                                                                                        $$PLSQL_UNIT);
                ...
                lib_proc.add(' ');
                loga('---ESTAB ' || cd.cod_estab ||
                     ' - PERIODO ENCERRADO: ' || v_retorno_status || '---',
                     FALSE);
        ...
        THEN
          ...
          IF v_validar_status = 1
          THEN
            -- ENCERRADO, RODA ISSO
          
            loga('PRC_UPD_NF_ENTRADA-INI', FALSE);
            prc_upd_nf_entrada(pdt_ini,
                               pdt_fim,
                               cd.cod_estab,
                               v_data_hora_ini);
            loga('PRC_UPD_NF_ENTRADA-FIM', FALSE);
      ...
      FOR cd IN lista_cds LOOP
        ...
        lib_proc.add(dsp_planilha.linha(p_conteudo => dsp_planilha.campo('COD_EMPRESA') --1 
                                                      ||
        ...
        FOR cr_r IN (SELECT *
          ...
          v_text01 := dsp_planilha.linha(p_conteudo => dsp_planilha.campo(cr_r.cod_empresa) -- 1
                                                       ||
                                                       dsp_planilha.campo(cr_r.cod_estab) -- 2
                                                       ||
                                                       dsp_planilha.campo(cr_r.data_fiscal) -- 3 
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.num_docfis)) -- 4
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.num_autentic_nfe)) -- 5
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.cod_fis_jur)) -- 6
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.cpf_cgc)) -- 7
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.cod_docto)) -- 8
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.cod_modelo)) -- 9
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.cod_cfo)) -- 10
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.natureza_operacao)) -- 11     
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.cod_produto)) -- 12 
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.descricao)) -- 13
                                                       ||
                                                       ...
                                                       ||
                                                       dsp_planilha.campo(dsp_planilha.texto(cr_r.num_item)) -- 18
                                                       ||
  ...
  BEGIN
    ...
    ELSE
      ...
      notifica('S', '', v_assunto, v_txt_email, $$PLSQL_UNIT);
  ...
  BEGIN
    ...
    v_sql := ' TRUNCATE TABLE DPSP_ENTRADA_NF_DP908_GTT ';
    ...
    v_qtde := 0;
    SELECT COUNT(1) INTO v_qtde FROM msafi.dpsp_entrada_nf_dp908_gtt;
    ...
    IF v_ok = 0 AND pdt_fim <= to_date('20161031', 'yyyymmdd')
    THEN
    
      --
    
      dbms_application_info.set_module($$PLSQL_UNIT, ' ENTRADA HIST');
      ...
      INSERT INTO msafi.dpsp_msaf_fin048_xml_gtt
        SELECT gtt.cod_empresa,
               ...
               nvl((SELECT ip.tax_brl_bse
                     FROM msafi.ps_ar_imp_bbl ip --msafi.ps_ar_imp_bbl_bkp_nov2016 ip
                    WHERE 1 = 1
               ...
               mproc_id,
               SYSDATE
          FROM --msafi.ps_ar_nfret_bbl_bkp_nov2016    capa,
               msafi.ps_ar_nfret_bbl    capa,
               --msafi.ps_ar_itens_nf_bbl_bkp_nov2016 i,
    ...
    dbms_application_info.set_module($$PLSQL_UNIT, ' ENTRADA 1');
...
/*      BEGIN
    ...
    SELECT COUNT(1) INTO v_qtde FROM msafi.dpsp_msaf_fin048_xml_gtt;
    ...
    dbms_application_info.set_module($$PLSQL_UNIT, ' ENTRADA - 2');
    ...
    INSERT INTO msafi.dpsp_msaf_fin048_xml_gtt
      SELECT /*+DRIVING_SITE(i)*/
       ...
       mproc_id,
       SYSDATE
        FROM msafi.ps_xml_forn xml_forn, msafi.dpsp_entrada_nf_dp908_gtt gtt
    ...
    dbms_application_info.set_module($$PLSQL_UNIT, ' ENTRADA  3');
    ...
    */
    FOR c IN (SELECT *
                FROM (SELECT /*+ parallel(32) */ x07.cod_empresa AS cod_empresa,
                             ...
                             REPLACE(aux.cfo_brl_cd, '.', '') AS cfop_saida_origem,
